...
public class TransformImpl implements Transform {
	...
	public void createParallelProcess(Map<String, Object> parameters) throws Exception {
		...
		try {
			...
			if (document.getHeader().getThemeID().equals("ARG_GW_PC_TarjetaAzul")) {
			document.getContent().getPolicyPeriod().deletePro();
			//	document.getContent().getPolicyPeriod();
			//String val = document.getContent().getPolicyPeriod().getCA7Line().getPrivatePassengers().getPrivatePassenger().getAdditionalInterestDetails();
			}else {
				...
				if (document.getHeader().getThemeID().equals(Constants.PC70)) {
					...
					document.getContent().getPolicyPeriod().getAAHLine().getAAHPartys().Comp70();
				}else{
